...
Sessions are stored in `~/.arc/sessions.json` as a JSON array. The web dashboard exposes sessions via the `/api/sessions` REST endpoint, with real-time status updates pushed over WebSocket.
